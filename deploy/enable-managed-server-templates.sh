...
text=text.replace("{tab === 'settings' && <SettingsPage server={server} />}", "{tab === 'settings' && <SettingsPage server={server} me={me} />}")
...
new="function SettingsPage({ server, me }: { server: Server; me: User }) {\n  const [reinstalling, setReinstalling] = useState(false);\n  const [reinstallMessage, setReinstallMessage] = useState('');\n  const managed = Boolean(server.template_slug && server.template_slug !== 'custom');\n  const canReinstall = me.is_admin || server.owner_id === me.id;\n  const reinstall = async () => {\n    if (!managed || !canReinstall || reinstalling) return;\n    if (!confirm('Geninstaller serveren? Nodexa gendanner template/Egg-filer som server.jar. Worlds, plugins og øvrige brugerfiler bevares. Serveren stoppes under geninstallationen.')) return;\n    setReinstalling(true); setReinstallMessage('');\n    try { const r = await axios.post(`/api/servers/${server.id}/reinstall`); setReinstallMessage(r.data?.message ?? 'Serveren er geninstalleret.'); }\n    catch (e: any) { setReinstallMessage(e?.response?.data?.error ?? e?.response?.data?.message ?? 'Geninstallationen fejlede.'); }\n    finally { setReinstalling(false); }\n  };\n  return <div className=\"module-stack\"><section className=\"module-heading\"><div><h2>Indstillinger</h2><p>Grundlæggende oplysninger for din server.</p></div></section><div className=\"settings-layout\"><div className=\"panel-card settings-card\"><div className=\"setting-block\"><label>Servernavn</label><div className=\"readonly-input\">{server.name}</div></div><div className=\"setting-block\"><label>Server ID</label><div className=\"readonly-input mono\">{server.identifier ?? server.id}</div></div><div className=\"setting-block\"><label>Template</label><div className=\"readonly-input\">{server.template_slug === 'minecraft-java' ? 'Minecraft Java' : server.template_slug || 'Custom'}</div></div><div className=\"setting-block\"><label>Ressourcer</label><div className=\"settings-resources\"><span>{fmtMb(server.memory_mb)} RAM</span><span>{fmtMb(server.disk_mb)} Disk</span><span>{server.cpu_limit || 0}% CPU</span></div></div></div><div className=\"panel-card info-card\"><div className=\"info-icon\"><Icon name=\"shield\"/></div><h3>Geninstaller server</h3><p>Som Pterodactyls Reinstall: Nodexa kører serverens template-installer igen og gendanner de styrede runtime-filer uden at slette worlds, plugins eller andre brugerfiler.</p>{reinstallMessage && <div className=\"info-note\">{reinstallMessage}</div>}{canReinstall && <button className=\"secondary-btn full\" disabled={!managed || reinstalling} onClick={reinstall}>{reinstalling ? 'Geninstallerer…' : managed ? 'Geninstaller server' : 'Ingen managed template'}</button>}<div className=\"info-note\">Startup command, Docker image og environment-variabler kan kun ændres af en administrator.</div></div></div></div>;\n}"
if old in text: text=text.replace(old,new,1)
p.write_text(text)
PY
fi

echo "[Nodexa] Managed server template UI enabled."
