...
if [[ -d "$AGENT_DIR" ]]; then
  ...
  if [[ -n "$AGENT_TOKEN" ]]; then log "Restarting configured Nodexa Agent..."; systemctl enable nodexa-agent >/dev/null 2>&1 || true; systemctl restart nodexa-agent; else log "Agent is not configured on this host; leaving nodexa-agent stopped."; systemctl disable --now nodexa-agent >/dev/null 2>&1 || true; fi
fi
...
if re.search(r'proxy_read_timeout\s+[^;]+;',text): text=re.sub(r'proxy_read_timeout\s+[^;]+;','proxy_read_timeout 210s;',text)
else: text=re.sub(r'(proxy_pass\s+[^;]+;)',r'\1\n        proxy_read_timeout 210s;',text,count=1)
if re.search(r'proxy_send_timeout\s+[^;]+;',text): text=re.sub(r'proxy_send_timeout\s+[^;]+;','proxy_send_timeout 210s;',text)
else: text=re.sub(r'(proxy_read_timeout\s+210s;)',r'\1\n        proxy_send_timeout 210s;',text,count=1)
p.write_text(text)
PY
fi
