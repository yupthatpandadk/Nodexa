...
<script>
$(document).on('click', '#deployCommand', function () {
    ...
    $('#configTokenBtn').on('click', function (event) {
        $.ajax({
            ...
            swal({
                type: 'success',
                title: 'Token created.',
                text: '<p>To auto-configure your node, click the command below to copy it:</p><pre id="deployCommand" style="cursor:pointer;white-space:pre-wrap;" title="Click to copy">cd /etc/pterodactyl && sudo wings configure --panel-url {{ config('app.url') }} --token ' + data.token + ' --node ' + data.node + '{{ config('app.debug') ? ' --allow-insecure' : '' }}</pre><small>Click command to copy</small>',
                html: true
            })
        }).fail(function () {
            swal({
