...
declare global { interface Window { __NODEXA_BOOTED__?: boolean; } }
...
function applyNodexaTheme(theme: any) {
  ...
  const values: Record<string, unknown> = {
    '--nodexa-primary': theme.primary, '--nodexa-secondary': theme.secondary,
    '--nodexa-background': theme.background, '--nodexa-surface': theme.surface,
    '--nodexa-text': theme.text, '--accent': theme.primary, '--primary': theme.primary,
    '--purple': theme.primary, '--bg': theme.background, '--surface': theme.surface, '--text': theme.text,
  };
...
async function syncNodexaTheme() {
  ...
  } catch (error) { console.warn('[Nodexa] Shared theme unavailable; using bundled defaults.', error); }
}
...
function responseArray(value: any): any[] {
  if (Array.isArray(value)) return value; if (Array.isArray(value?.data)) return value.data;
  if (Array.isArray(value?.items)) return value.items; if (value?.data && typeof value.data === 'object') return responseArray(value.data); return [];
}
...
axios.interceptors.response.use(response => {
  ...
  if (/\/api\/servers\/[^/]+\/users/.test(url)) { const container = Array.isArray(response.data) ? response.data : Array.isArray(response.data?.data) ? response.data.data : null; if (container) for (const entry of container) if (entry && typeof entry === 'object' && entry.user) entry.user = normalizeUser(entry.user); }
  return response;
}, error => {
...
function NodexaRoot() {
  ...
  return <NodexaErrorBoundary><Router><App /></Router></NodexaErrorBoundary>;
}
try {
