...
export default function App() {
  ...
  useEffect(() => { if (me?.is_admin && view === 'nodes') loadNodes(); }, [me, view]);
...
function ServerWorkspace(props: { me: User; server: Server; stats: Stats | null; tab: Tab; setTab: (t: Tab) => void; back: () => void; power: (s: string) => any; logs: string; cmd: string; setCmd: (v: string) => void; send: () => void; databases: Database[]; dbName: string; setDbName: (v: string) => void; createDatabase: () => void; openDatabase: (d: Database) => void; deleteDatabase: (d: Database) => void; password: string | null; clearPassword: () => void; pmaUrl: string | null; closePma: () => void; users: Subuser[]; addUser: (e: string, p: string[]) => any; updateUser: (u: Subuser, p: string[]) => any; removeUser: (u: Subuser) => any }) {
  ...
  return <div className="page-stack server-workspace">
    ...
    {tab === 'files' && <div className="module-stack">{has(server, 'files.sftp') && <SftpAccessCard server={server}/>}<ModulePlaceholder icon="folder" title="Filer" text="Filhåndtering bliver koblet direkte på Nodexa Agent med editor, upload, download og arkivfunktioner." /></div>}
...
const permissionGroups = [
  ...
  { title: 'Filer & SFTP', items: [['files.read','Se filer'],['files.write','Rediger filer'],['files.sftp','Brug SFTP']] },
...
function UsersPage({ users, add, update, remove }: { users: Subuser[]; add: (e: string, p: string[]) => any; update: (u: Subuser, p: string[]) => any; remove: (u: Subuser) => any }) {
  const [showInvite, setShowInvite] = useState(false); const [editing, setEditing] = useState<Subuser | null>(null); const [email, setEmail] = useState(''); const [permissions, setPermissions] = useState<string[]>(['console.read']); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const toggle = (p: string) => setPermissions(v => v.includes(p) ? v.filter(x => x !== p) : [...v, p]);
  const closeModal = () => { setShowInvite(false); setEditing(null); setEmail(''); setPermissions(['console.read']); setError(''); };
  const openInvite = () => { setEditing(null); setEmail(''); setPermissions(['console.read']); setError(''); setShowInvite(true); };
  const openEdit = (entry: Subuser) => { setShowInvite(false); setEditing(entry); setPermissions(toStringArray(entry.permissions)); setError(''); };
  const submit = () => { setBusy(true); setError(''); const request = editing ? update(editing, permissions) : add(email.trim(), permissions); Promise.resolve(request).then(closeModal).catch(e => setError(e?.response?.data?.message ?? (editing ? 'Kunne ikke gemme rettighederne.' : 'Kunne ikke invitere brugeren.'))).finally(() => setBusy(false)); };
  return <div className="module-stack"><section className="module-heading"><div><h2>Brugere</h2><p>Giv andre eksisterende Nodexa-brugere adgang til denne server.</p></div><button className="primary-btn" onClick={openInvite}><Icon name="plus"/> Inviter bruger</button></section>
    <div className="panel-card user-list"><div className="owner-row"><div className="user-identity"><div className="avatar owner-avatar">E</div><div><strong>Serverejer</strong><span>Fuld adgang til serveren</span></div></div><span className="role-badge">Ejer</span><div className="permission-summary"><Icon name="shield" size={15}/> Alle rettigheder</div></div>{users.map(entry => { const p = toStringArray(entry.permissions); return <div className="user-row" key={entry.id}><div className="user-identity"><div className="avatar">{initials(entry.user.name)}</div><div><strong>{entry.user.name}</strong><span>{entry.user.email}</span></div></div><span className="role-badge secondary">Bruger</span><div className="permission-summary">{p.length} rettigheder</div><div className="user-actions"><button className="secondary-btn small" onClick={() => openEdit(entry)}>Rediger</button><button className="danger-link" onClick={() => confirm(`Fjern ${entry.user.email}?`) && remove(entry)}>Fjern</button></div></div>; })}{!users.length && <div className="subtle-empty">Ingen ekstra brugere har adgang til serveren endnu.</div>}</div>
    {(showInvite || editing) && <div className="modal"><div className="modal-card invite-modal"><div className="modal-title"><div><div className="eyebrow">SERVER ACCESS</div><h2>{editing ? 'Rediger adgang' : 'Inviter bruger'}</h2><p>{editing ? `Vælg rettigheder for ${editing.user.email}.` : 'Brugeren skal allerede have en Nodexa-konto.'}</p></div><button className="close-btn" onClick={closeModal}>×</button></div>{!editing && <label className="field-label">E-mailadresse<input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="ven@eksempel.dk"/></label>}<div className="permissions-grid">{permissionGroups.map(group => <div className="permission-group" key={group.title}><strong>{group.title}</strong>{group.items.map(([key,label]) => <label key={key}><input type="checkbox" checked={permissions.includes(key)} onChange={() => toggle(key)}/><span>{label}</span></label>)}</div>)}</div>{error && <div className="auth-error">{error}</div>}<div className="modal-actions"><button className="secondary-btn" disabled={busy} onClick={closeModal}>Annuller</button><button className="primary-btn" disabled={busy || permissions.length === 0 || (!editing && !email.trim())} onClick={submit}>{busy ? 'Gemmer…' : editing ? 'Gem rettigheder' : 'Giv adgang'}</button></div></div></div>}
  </div>;
}
...
function NodesPage({ nodes, showForm, setShowForm, form, setForm, createNode, configFor, config, closeConfig }: { nodes: Node[]; showForm: boolean; setShowForm: (v: boolean) => void; form: any; setForm: (v: any) => void; createNode: () => void; configFor: (n: Node) => void; config: NodeConfig | null; closeConfig: () => void }) {
  ...
  return <div className="page-stack"><section className="page-heading-row"><div><div className="eyebrow">ADMINISTRATION</div><h1>Nodes</h1><p>Administrer Nodexa Agent-noder og deres kapacitet.</p></div><button className="primary-btn" onClick={() => setShowForm(true)}><Icon name="plus"/> Opret Node</button></section><div className="node-grid">{nodes.map(node => <div className="panel-card node-card" key={node.id}><div className="node-top"><div className="node-icon"><Icon name="node"/></div><span className="online-badge"><span/> Online</span></div><h3>{node.name}</h3><p>{node.location ?? 'Ingen lokation'}</p><div className="node-host">{node.fqdn}:{node.daemon_port}</div><div className="node-stats"><div><span>RAM</span><strong>{fmtMb(node.memory_mb)}</strong></div><div><span>Disk</span><strong>{fmtMb(node.disk_mb)}</strong></div><div><span>Servere</span><strong>{node.servers_count ?? 0}</strong></div></div><button className="secondary-btn full" onClick={() => { setShowConfigSecret(false); configFor(node); }}>Konfiguration</button></div>)}{!nodes.length && <div className="panel-card node-empty"><div className="empty-icon"><Icon name="node"/></div><h3>Ingen Nodes endnu</h3><p>Opret din første Node og installer Nodexa Agent.</p><button className="primary-btn" onClick={() => setShowForm(true)}><Icon name="plus"/> Opret Node</button></div>}</div>
