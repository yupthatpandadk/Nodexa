...
export default function App(){
 const [me,setMe]=useState<User|null>(null),[servers,setServers]=useState<Server[]>([]),[selected,setSelected]=useState<Server|null>(null);
 ...
 useEffect(()=>{
  axios.get('/api/me').then(r=>setMe(r.data)).catch(()=>{});
  axios.get('/api/servers').then(r=>{const s=r.data.data??r.data;setServers(s);if(s[0])setSelected(s[0]);});
 },[]);
 ...
 useEffect(()=>{if(selected&&tab==='databases')loadDatabases();},[selected,tab]);
 ...
 const openDatabase=(db:Database)=>selected&&axios.post(`/api/servers/${selected.id}/databases/${db.id}/open`).then(r=>window.open(r.data.url,'_blank','noopener'));
 ...
 return <div className="shell">
  <aside>
   ...
   <nav>
    <button className={tab!=='nodes'?'active':''} onClick={()=>setTab('console')}>My Servers</button>
    {me?.is_admin&&<><div className="nav-label">ADMINISTRATION</div><button className={tab==='nodes'?'active':''} onClick={()=>setTab('nodes')}>Nodes</button><button>All Servers</button><button>Allocations</button><button>Users</button></>}
   </nav>
   <div className="account"><strong>{me?.name??'Nodexa User'}</strong><small>{me?.is_admin?'Administrator':'Customer'}</small></div>
  ...
  <main>
   {tab==='nodes'&&me?.is_admin?<NodesPage nodes={nodes} showForm={showNodeForm} setShowForm={setShowNodeForm} form={nodeForm} setForm={setNodeForm} createNode={createNode} configFor={configFor} config={nodeConfig} closeConfig={()=>setNodeConfig(null)}/>:<>
    <header><div><small>NODEXA · {selected?.identifier??'SERVER'}</small><h1>{selected?.name??'My Servers'}</h1></div><div className={'status '+(stats?.state==='running'?'online':'offline')}>{stats?.state??selected?.status??'offline'}</div></header>
    <section className="server-grid">
     <div className="server-list card"><h3>My Servers</h3>{servers.map(s=><button key={s.id} className={selected?.id===s.id?'active':''} onClick={()=>{setSelected(s);setTab('console')}}><strong>{s.name}</strong><small>{s.identifier??'—'} · {s.memory_mb} MB</small></button>)}</div>
     <div className="content">
      ...
      {tab==='console'&&<ConsolePage stats={stats} logs={logs} cmd={cmd} setCmd={setCmd} send={send} power={power}/>} 
      {tab==='databases'&&selected&&<DatabasePage server={selected} databases={databases} dbName={dbName} setDbName={setDbName} createDatabase={createDatabase} openDatabase={openDatabase} deleteDatabase={deleteDatabase} password={newPassword} clearPassword={()=>setNewPassword(null)}/>} 
      {tab!=='console'&&tab!=='databases'&&<div className="card placeholder"><h2>{tab[0].toUpperCase()+tab.slice(1)}</h2><p>This section is connected to the Nodexa server navigation and ready for its runtime module.</p></div>}
     </div>
    </section>
   </>}
  </main>
