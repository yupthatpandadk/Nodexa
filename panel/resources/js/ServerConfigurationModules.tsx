...
export function SettingsPage({server,canUpdate=true,canReinstall=true,canSftp=true}:{server:ServerLike;canUpdate?:boolean;canReinstall?:boolean;canSftp?:boolean}){
  ...
  const reinstall=async()=>{if(!canReinstall||!confirm('Geninstaller serveren? Template-filer installeres igen.'))return;setBusy(true);setError('');setMessage('');try{await axios.post(`/api/servers/${server.id}/reinstall`);setMessage('Geninstallationen er startet.')}catch(e){setError(err(e))}finally{setBusy(false)}};
