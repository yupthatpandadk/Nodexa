...
export default ({ embedded = false }: MinecraftPlayerListProps) => {
    ...
    const body = status !== 'running' ? (
        <div className={'flex flex-1 items-center justify-center p-8 text-center text-sm text-gray-400'}>Serveren skal være startet for at vise spillere.</div>
    ) : players.length === 0 ? (
        <div className={'flex flex-1 flex-col items-center justify-center p-8 text-center'}>
            <div className={'text-4xl mb-3'}>👥</div>
            <div className={'font-semibold text-gray-200'}>{loading ? 'Henter spillerliste…' : 'Ingen spillere online'}</div>
            <div className={'text-xs text-gray-500 mt-1'}>{updated ? 'Spillere vises her, når de joiner serveren.' : 'Venter på svar fra Minecraft-serveren…'}</div>
        </div>
    ) : (
        <div className={'overflow-y-auto flex-1'}>
            {players.map(player => (
                <div key={player} className={'px-4 py-3 flex flex-wrap items-center justify-between gap-3 border-t'} style={{ borderColor: 'var(--nodexa-border)' }}>
                    <div className={'flex items-center gap-3 min-w-0'}>
                        <img src={`https://mc-heads.net/avatar/${encodeURIComponent(player)}/40`} width={40} height={40} className={'rounded-lg'} alt={player}/>
                        <div className={'min-w-0'}><div className={'font-semibold text-white truncate'}>{player}</div><div className={'text-xs text-green-400'}>● Online</div></div>
                    </div>
                    {canCommand && <div className={'flex flex-wrap gap-1.5'}><button onClick={() => command('kick', player)} className={'px-2 py-1 rounded bg-gray-700 text-white text-xs'}>Kick</button><button onClick={() => command('op', player)} className={'px-2 py-1 rounded bg-gray-700 text-white text-xs'}>OP</button><button onClick={() => command('deop', player)} className={'px-2 py-1 rounded bg-gray-700 text-white text-xs'}>De-OP</button><button onClick={() => command('whitelist add', player)} className={'px-2 py-1 rounded bg-gray-700 text-white text-xs'}>Whitelist</button><button onClick={() => command('ban', player)} className={'px-2 py-1 rounded bg-red-700 text-white text-xs'}>Ban</button></div>}
                </div>
            ))}
        </div>
    );
    ...
    if (embedded) return (
        <section className={'rounded-xl overflow-hidden flex flex-col min-h-[430px]'} style={{ background: 'var(--nodexa-card)', border: '1px solid var(--nodexa-border)' }}>
            <div className={'px-4 py-3 flex items-center justify-between gap-3'} style={{ borderBottom: '1px solid var(--nodexa-border)' }}>
                <div className={'flex items-center gap-2'}><span>👤</span><h2 className={'font-header font-semibold text-gray-100'}>Players</h2><span className={'text-sm text-gray-400'}>{online} / {max ?? '—'}</span></div>
                <button onClick={refresh} disabled={loading || status !== 'running'} className={'text-xs font-semibold disabled:opacity-40'} style={{ color: 'var(--nodexa-accent)' }}>{loading ? 'Opdaterer…' : '↻ Opdater'}</button>
            </div>
            {body}
            <div className={'grid grid-cols-2 gap-2 px-4 py-3 text-center'} style={{ borderTop: '1px solid var(--nodexa-border)' }}>
                <div><div className={'text-white font-semibold'}>{online} / {max ?? '—'}</div><div className={'text-xs text-gray-500'}>Players</div></div>
                <div><div className={'text-white font-semibold'}>{updated ? updated.toLocaleTimeString() : '—'}</div><div className={'text-xs text-gray-500'}>Senest opdateret</div></div>
            </div>
        </section>
    );
