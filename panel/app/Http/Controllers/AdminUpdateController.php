...
class AdminUpdateController extends Controller
{
    ...
    public function check(Request $request)
    {
        ...
        try{$remote=$this->remote();$installedCommit=trim((string)($installed['commit']??''));$remoteCommit=trim((string)($remote['commit']??''));$known=$installedCommit!==''&&$remoteCommit!=='';return ['available'=>$known&&$installedCommit!==$remoteCommit,'installation_known'=>$known,'installed'=>['version'=>$installed['version']??'unknown','commit'=>$installedCommit?:null,'short_commit'=>$installedCommit!==''?substr($installedCommit,0,8):null,'installed_at'=>$installed['installed_at']??null],'latest'=>$remote,'state'=>$this->readJson(self::STATE_FILE)];}
        catch(Throwable $e){SystemIssue::report(source:'updater',title:'Kunne ikke kontrollere Nodexa-opdateringer',message:$e->getMessage(),severity:'warning',type:'update_check_failed');return response()->json(['message'=>'Kunne ikke kontakte GitHub for at kontrollere opdateringer.','installed'=>$installed,'state'=>$this->readJson(self::STATE_FILE)],503);}
    }
