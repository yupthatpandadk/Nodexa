...
class PaymentWebhookController extends Controller {
 public function handle(Request $request,string $gateway,PaymentCompletionService $payments){
  ...
  try{$result=$payments->complete($invoiceId,$gateway,$transaction,isset($data['amount'])?(float)$data['amount']:null,$data);return response()->json(['ok'=>true,'already_paid'=>$result['already_paid'],'provisioned'=>$result['provisioned'],'provisioning_pending'=>!$result['provisioned']&&$result['provision_error']!==null]);}catch(\Throwable $e){report($e);return response()->json(['error'=>'Betalingen kunne ikke færdigbehandles'],422);}
 }
}
