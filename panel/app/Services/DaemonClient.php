...
final class DaemonClient
{
    private function client(Node $node, int $timeout = 8): PendingRequest
    {
        return Http::baseUrl(sprintf('%s://%s:%d', $node->scheme, $node->fqdn, $node->daemon_port))
            ->withToken($node->token)->acceptJson()->connectTimeout(3)->timeout($timeout);
    }
    ...
    private function guarded(Node $node, ?string $serverId, string $action, callable $callback): mixed
    {
        try {
            ...
            try { SystemIssue::report(source:'agent',title:$title,message:$message,severity:$severity,type:$type,nodeId:$node->id,serverId:$serverId,context:array_merge(['action'=>$action,'node'=>$node->name,'host'=>$node->fqdn,'port'=>$node->daemon_port,'scheme'=>$node->scheme,'exception'=>get_class($e)],$extra)); } catch (Throwable) {}
            throw $e;
        }
    }
    ...
    {
        return ['id'=>$server->uuid,'name'=>$server->name,'image'=>$server->docker_image,'startup'=>$server->startup,'template'=>$server->template_slug??'custom','memory_mb'=>$server->memory_mb,'disk_mb'=>$server->disk_mb,'cpu_limit'=>$server->cpu_limit,'environment'=>$server->environment??[]];
    }

    public function createServer($server): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'create_server',fn()=>$this->client($node,180)->post('/api/servers',$this->serverPayload($server))->throw()->json()); }
    public function reinstall($server): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'reinstall_server',fn()=>$this->client($node,180)->post("/api/servers/{$server->uuid}/reinstall",$this->serverPayload($server))->throw()->json()); }
    ...
    public function renameFile($server,string $from,string $to): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'rename_file',fn()=>$this->client($node)->post("/api/servers/{$server->uuid}/files/rename",['from'=>$from,'to'=>$to])->throw()->json()); }
    public function uploadFile($server,string $directory,\Illuminate\Http\UploadedFile $file): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'upload_file',function()use($node,$server,$directory,$file){$handle=fopen($file->getRealPath(),'rb');try{return $this->client($node,180)->attach('file',$handle,$file->getClientOriginalName())->post("/api/servers/{$server->uuid}/files/upload",['path'=>$directory])->throw()->json();}finally{if(is_resource($handle))fclose($handle);}}); }
    public function downloadFile($server,string $path): string { $node=$server->node; return $this->guarded($node,(string)$server->id,'download_file',fn()=>$this->client($node,120)->get("/api/servers/{$server->uuid}/files/download",['path'=>$path])->throw()->body()); }
    public function archive($server,string $path): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'archive_file',fn()=>$this->client($node,120)->post("/api/servers/{$server->uuid}/files/archive",['path'=>$path])->throw()->json()); }
    public function extract($server,string $path): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'extract_file',fn()=>$this->client($node,120)->post("/api/servers/{$server->uuid}/files/extract",['path'=>$path])->throw()->json()); }
    ...
    public function backups($server): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'list_backups',fn()=>$this->client($node)->get("/api/servers/{$server->uuid}/backups")->throw()->json()); }
    public function backup($server,string $name): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'backup',fn()=>$this->client($node,180)->post("/api/servers/{$server->uuid}/backups",['name'=>$name])->throw()->json()); }
    public function downloadBackup($server,string $name): string { $node=$server->node; return $this->guarded($node,(string)$server->id,'download_backup',fn()=>$this->client($node,180)->get("/api/servers/{$server->uuid}/backups/".rawurlencode($name))->throw()->body()); }
    public function restoreBackup($server,string $name): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'restore_backup',fn()=>$this->client($node,180)->post("/api/servers/{$server->uuid}/backups/".rawurlencode($name).'/restore')->throw()->json()); }
    public function deleteBackup($server,string $name): array { $node=$server->node; return $this->guarded($node,(string)$server->id,'delete_backup',fn()=>$this->client($node)->delete("/api/servers/{$server->uuid}/backups/".rawurlencode($name))->throw()->json()); }
}
