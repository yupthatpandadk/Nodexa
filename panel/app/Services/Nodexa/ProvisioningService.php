...
class ProvisioningService {
 public function provisionOrder(int $orderId):array {
  ...
  $cfg=json_decode((string)($product->provisioning??''),true)?:[]; $productId=(string)($product->provider_product_id??$cfg['product_id']??''); $osId=(string)($existing->os_id??$product->default_os_id??$cfg['os_id']??'');
  if($productId===''||$osId==='')throw new RuntimeException('VPS-pakken mangler provider product ID eller valgt operativsystem.');
  ...
  $serviceId=$existing?->id; if(!$serviceId)$serviceId=DB::table('vps_services')->insertGetId(['user_id'=>$order->user_id,'product_id'=>$product->id,'provider_id'=>$provider->id,'order_id'=>$orderId,'provider_server_id'=>null,'hostname'=>null,'ip_address'=>null,'os_id'=>$osId,'status'=>'provisioning','meta'=>json_encode(['started_at'=>now()->toIso8601String()]),'created_at'=>now(),'updated_at'=>now()]);else DB::table('vps_services')->where('id',$serviceId)->update(['provider_id'=>$provider->id,'os_id'=>$osId,'status'=>'provisioning','updated_at'=>now()]);
  try{$api=new FlaxVpsClient($provider);$response=$api->create($productId,$osId);$remote=(string)($response['id']??$response['server_id']??$response['serverId']??$response['uuid']??$response['service_id']??'');if($remote==='')throw new RuntimeException('Flax oprettede VPS’en, men returnerede ikke et server-ID.');$meta=json_decode((string)($existing->meta??''),true)?:[];$meta['started_at']=$meta['started_at']??now()->toIso8601String();$meta['provisioned_at']=now()->toIso8601String();$meta['provider_response']=$response;DB::table('vps_services')->where('id',$serviceId)->update(['provider_server_id'=>$remote,'status'=>'provisioning','meta'=>json_encode($meta),'updated_at'=>now()]);DB::table('billing_orders')->where('id',$orderId)->update(['status'=>'completed','updated_at'=>now()]);return ['service'=>DB::table('vps_services')->where('id',$serviceId)->first(),'already_provisioned'=>false];}
  catch(\Throwable $e){$meta=json_decode((string)($existing->meta??''),true)?:[];$meta['failed_at']=now()->toIso8601String();$meta['error']=$e->getMessage();DB::table('vps_services')->where('id',$serviceId)->update(['status'=>'provision_failed','meta'=>json_encode($meta),'updated_at'=>now()]);DB::table('billing_orders')->where('id',$orderId)->update(['status'=>'paid','updated_at'=>now()]);throw $e;}
 }
}
